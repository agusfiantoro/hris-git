...
use App\Models\Eletter\MasterEletter\SternWarningLetter;
use PDF;
use SimpleSoftwareIO\QrCode\Facades\QrCode;

class SternWarningLetterController extends Controller
{
	public function index(Request $request)
	{
		if ($request->ajax()) {
			$data = SternWarningLetter::get_swp($request);
			...
			->addColumn('action', function($data) {
				...
				$button .= '<a href="'.route('print.sp',$data->token).'" target="_blank" name="print" id="" class="btn btn-success btn-sm btn-print" title="Print"><span class="fa fa-file-pdf"></span></a><br>';
		...
		SternWarningLetter::cek_exp_spdt();
		return view('eletter.stern_warning_letter.index');
	}
	public function get_data(Request $request)
	{
		$category = SternWarningLetter::get_category();
		$location = SternWarningLetter::get_location();
		$employee = SternWarningLetter::get_employee_chief();
	...
	public function save(Request $request)
	{
		...
		try {
			DB::beginTransaction();
			$result = SternWarningLetter::format_save($request);
