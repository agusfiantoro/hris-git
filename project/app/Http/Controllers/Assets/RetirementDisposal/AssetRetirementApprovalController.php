...
class AssetRetirementApprovalController extends Controller {	
    ...
    public function index(Request $request) {
        if($request->ajax()) {
            ...
            try {
                $data = $this->getApprovalData('Asset_Retirement', $employee->id_employee, "Request_Approval");
            } catch(Exception $e) {
                $data = $this->getApprovalDataLegacy('Retirement', $employee->id_employee);
            }
            
            return DataTables::of($data)
                            ->addIndexColumn()
                            ->addColumn('action', function ($row) {
                                return @$row->id_retirement_header ?? $row->id_source_transaction;
                            })
                            ->make();
        }
        return view('assets.retirement_disposal.retirement_approval.index');
    }
    ...
    public function approve(Request $request) {
        ...
        DB::beginTransaction();
        try {
            $header = RetirementHeader::findOrFail($request->id_retirement_header);
            $employee = HrEmployee::where('id_user', session('id_user'))->active()->first();
            $approval = $this->assetApproval($header, $employee->id_employee, "Approved");

            DB::commit();
            return $this->success($approval, "Retirement approval success!");
        } catch(Exception $e) {
            DB::rollBack();
    ...
    public function reject(Request $request) {
        ...
        try {
            ...
            DB::commit();
            return $this->success($approval, "Retirement has been rejected!");
        } catch(Exception $e) {
            DB::rollBack();
            return $this->error(null, $e->getMessage());
        }
