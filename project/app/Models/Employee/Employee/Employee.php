...
*/
	...
	public static function getdata($group_branch, $nik=null, $status=null) {
		$get = DB::table(DB::raw("sp_funct_get_employee ('".session('id_company')."') se"))
				->join('hr_employee as he', 'se.id_employee', '=', 'he.id_employee')
				->leftJoin('web.hr_candidate as hc', 'he.identification_number', '=', 'hc.identification_number')
                ->select('hc.id_candidate', 'hc.is_profil_completed', 'se.*');
	...
	*/
	public static function getreport($group_branch) {
		...
		$sql = "SELECT 
                sr.*,
                hde_sim.attachment AS sim_attachment,
                hde_npwp.attachment AS npwp_attachment,
                hde_br.attachment AS buku_rekening_attachment,
                mc.company_name AS assigned_company
			FROM sp_funct_get_employee_report_all(".session('id_company').") sr 
            LEFT JOIN hr_document_employee hde_sim ON sr.id_employee = hde_sim.id_employee AND hde_sim.document_name = 'SIM'
            LEFT JOIN hr_document_employee hde_npwp ON sr.id_employee = hde_npwp.id_employee AND hde_npwp.document_name = 'NPWP'
            LEFT JOIN hr_document_employee hde_br ON sr.id_employee = hde_npwp.id_employee AND hde_npwp.document_name = 'Buku Rekening'
            LEFT JOIN master_position_detail mpd ON sr.id_position_detail = mpd.id_position_detail
            LEFT JOIN master_company mc ON mpd.id_company = mc.id_company
            ".$where_branch;
        $data = DB::select($sql);
        return $data;
    }
	...
	*/
	...
	public static function getEmployeeDetail($nik=null, $idEmployee=null, $full=false, $isActive=true, $parentIdPositionDetail=false) {
		...
		$q1->where('mpd.id_company', session('id_company'));
        $result = $q1->get();
        if($result->count() > 0){
