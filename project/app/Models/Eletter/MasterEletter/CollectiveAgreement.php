...
class CollectiveAgreement extends Model
{
	public static function change_employee_pb($request)
	{
		if ($request->employee_status == "A") {
			...
			->select(
				...
				\DB::RAW('position_routing_old.id_routing as id_routing')
			)
	...
	// 			->leftJoin('master_general_data as data_pb','data_pb.id_general_data','=','data_category.relation_to_id_general_data')
	// 			->whereColumn('hr_electronic_letter.id_employee', '=', 'hr_employee.id_employee')
	// 			->where('data_pb.code','PB');
	...
	// 	return $employee;
	// }
	public static function get_employee_pb($request)
	{
		$id_branch = ElectronicLetter::accessBranch($request);
