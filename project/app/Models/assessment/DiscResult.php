...
class DiscResult extends Model
{
    ...
    protected $table = 'web.psycho_disc_result_scoring';
    protected $primaryKey = 'id_disc_result_scoring';
    protected $fillable = [
        'id_candidate','id_employee','id_batch','character','value','job_match','status','id_company','creation_date','update_date','created_by','updated_by'
    ];

    public static function getData($idBatch=null, $idCandidateOrIdEmployee=null, $type='candidate') {
        $get = DB::table('web.psycho_disc_result_scoring as pdrc');
        if($idBatch){
    ...
    public static function getAllData($idBatch=null, $idCandidateOrIdEmployee=null, $type='candidate', $start=null, $end=null) {
        ...
        if($start && $end){
            $get->whereBetween('pdrc.creation_date', [$start, $end]);
        }
        $return = $get->get();
