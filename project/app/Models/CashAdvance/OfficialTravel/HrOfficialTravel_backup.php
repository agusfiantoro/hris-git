...
class HrOfficialTravel extends Model
{
	...
	public static function get_offtrav() // index
	{
		...
		WHERE hr_official_travel.id_company = ".$id_company." AND hr_approval_transaction.source_transaction_type='Official_Travel' AND hr_approval_transaction.id_company=".$id_company." AND hr_employee.id_user=".session('id_user')." ORDER BY hr_official_travel.id_official_travel DESC
		";
		$data = DB::select($data);
		return $data;
	}
	
	public static function get_offtrav_summary() // index
	{
		$id_company = session('id_company');
		$data = "SELECT hr_official_travel.id_official_travel as id_official_travel, hr_official_travel.reference_number as reference_number, hr_official_travel.letter_date as letter_date, hr_employee.name as name, master_position_routing.description as dec_position,
		hr_official_travel.start_date as start_date, hr_official_travel.end_date as end_date, hr_official_travel.location_from as location_from, hr_official_travel.location_to as location_to, master_general_data.description as category, hr_official_travel.id_reason_group as id_reason_group, hr_official_travel.reason_notes as reason_notes, hr_official_travel.unlock_gps as unlock_gps, hr_official_travel.is_have_cash_advance as is_have_cash_advance, approval_status.code as code_app_status, approval_status.code as status_approval, approval_status.description as desc_app_status, hr_official_travel.travel_status,
		...
		LEFT JOIN hr_employee as approval_request ON approval_request.id_employee=hr_official_travel.id_approval_request
		LEFT JOIN hr_approval_transaction ON hr_approval_transaction.id_source_transaction=hr_official_travel.id_official_travel
		WHERE hr_official_travel.id_company = ".$id_company." AND hr_approval_transaction.source_transaction_type='Official_Travel' AND hr_approval_transaction.id_company=".$id_company." 
	...
	public static function get_region_destination($request)
	{
		$id_user = session('id_user');
		$id_company = session('id_company');
		$code_product = $request->code_product;
		$data = "SELECT DISTINCT master_region.id_region as id, master_region.description as text, master_grade_expenses.max_price as max_price, master_region.region_code as code_region
		...
		WHERE hr_employee.id_user='$id_user' AND inventory.master_product.code='$code_product' AND hr_employee.id_company='$id_company' AND master_grade_expenses.status='A' ORDER BY master_region.description ASC
		";
		$data = DB::select($data);
		return $data;
	}
	// cashadvance
	public static function get_branch_destination($request)
	{
		$id_user = session('id_user');
		$id_company = session('id_company');
		...
		$data = "SELECT DISTINCT master_grade_expenses.id_grade_expense as id, master_grade_expenses.description as text, master_grade_expenses.max_price as max_price FROM master_grade_expenses
		LEFT JOIN master_job_grade ON master_job_grade.id_job_grade=master_grade_expenses.id_job_grade
		LEFT JOIN inventory.master_product ON inventory.master_product.id_product=master_grade_expenses.id_product AND inventory.master_product.status = 'A'
		LEFT JOIN inventory.master_product_categories ON inventory.master_product_categories.id_product_categories=master_product.id_product_categories
		LEFT JOIN master_company ON master_company.id_company=master_grade_expenses.id_company
		...
		LEFT JOIN hr_employee ON (hr_employee.id_employee=master_position_detail.id_employee OR hr_employee.id_employee=master_position_detail.id_employee2)
		LEFT JOIN master_region ON master_region.id_region=master_grade_expenses.id_region
		WHERE hr_employee.id_user='$id_user' AND hr_employee.id_company='$id_company' AND inventory.master_product.code='$code_product' $filter_status AND master_grade_expenses.id_region='$id_region' ORDER BY master_grade_expenses.description ASC
		";
		$data = DB::select($data);
		return $data;
	}
	...
	public static function getDirector($columns = "he.id_employee id, he.name text, mu.id_user as id_user", $custom_id_user = null) {
		...
		where mjg.job_level = 1 and mjg.id_company = ? and he.id_company = ? and mpr.parent_id_routing isnull";
		if($custom_id_user) {
			$director .= " and mu.id_user = ".$custom_id_user;
		} 
		return DB::selectOne($director, [session('id_company'), session('id_company')]);
	} 

	public static function getDirectorOfficialTravel($columns = "hah.id_approval id, hah.description text, hah.id_job_grade, hah.is_auto_approved") {
		...
		WHERE mgd.code = 'Official_Travel' and hah.id_company = ? AND hah.status = 'A'", [session('id_company')]);
	}

	public function get_approval_by($request)
	{
		...
		// $is_director = "SELECT *, had.id_employee as id_director FROM hr_approval_header hah 
		// JOIN master_general_data mgd ON hah.id_approval_doc_type = mgd.id_general_data
		// JOIN hr_approval_detail had ON hah.id_approval = had.id_approval
		// JOIN master_job_grade mjg ON hah.id_job_grade = mjg.id_job_grade AND mjg.job_level = 1 AND hah.is_auto_approved = true AND mjg.status = 'A'
		// JOIN hr_employee he ON had.id_employee = he.id_employee
		// WHERE mgd.code = 'Official_Travel' and hah.id_company = ? AND hah.status = 'A' AND he.id_user = ?";
		// $is_director = DB::selectOne($is_director, [session('id_company'), session('id_user')]);

		if($director && $director->id_user != session('id_user')) {
			$sql_approval_by = "SELECT he2.id_employee id, he2.name text 
			FROM (
				SELECT (
					...
					LEFT JOIN master_general_type mgt
					ON mgd.id_general_type = mgt.id_general_type AND mgt.general_type = 'master_approval_doc_type'
					WHERE mgd.id_company = '1' AND mgd.code='Official_Travel'
				), sfao.id_approval_mode, sfao.sequence, sfao.id_detail_chief, sfao.description_chief, sfao.id_employee_approval, he.name
				FROM (
					select sfao.* from sp_funct_approval_organization_hierarchy_view (".$request->id_employee.",'1',null,Array['gm-level','manager-level']) sfao
					left join master_position_detail mpd on mpd.parent_id_position_detail = sfao.id_detail_chief
					where mpd.secondary_position = false and mpd.id_employee = ".$request->id_employee." and mpd.status = 'A' and mpd.id_company = ".session('id_company')."
				) as sfao
			...
			LEFT JOIN master_general_type mgt
			ON mgd.id_general_type = mgt.id_general_type AND mgt.general_type = 'master_approval_doc_type'
			WHERE mgd.id_company = ".session('id_company')." AND mgd.code='Official_Travel' and hah.status = 'A' and hah.id_job_grade isnull and hah.is_auto_approved = false";
		} else {
			// $sql_approval_by = "SELECT he.id_employee id, he.name text, mpr.* from hr_employee he 
			// join master_position_detail mpd on he.id_employee = mpd.id_employee and mpd.status = 'A'
			// join master_position_routing mpr on mpd.id_position_routing = mpr.id_routing and mpr.status = 'A'
			// join master_job_grade mjg on mpr.id_job_grade = mjg.id_job_grade 
			// where mjg.job_level = 1 and mjg.id_company = ".session('id_company')." and he.id_company = ".session('id_company')." and mpr.parent_id_routing isnull";
			$sql_approval_by = [$director];
			$sql_approval_hirarki = "SELECT hah.id_approval id, hah.description text FROM hr_approval_header hah 
			JOIN master_general_data mgd ON hah.id_approval_doc_type = mgd.id_general_data
			JOIN hr_approval_detail had ON hah.id_approval = had.id_approval
			JOIN master_job_grade mjg ON hah.id_job_grade = mjg.id_job_grade AND mjg.job_level = 1 AND hah.is_auto_approved = true AND mjg.status = 'A'
			WHERE mgd.code = 'Official_Travel' and hah.id_company = ".session('id_company')." AND hah.status = 'A'";
		}
		...
		// WHERE mgd.id_company = '".session('id_company')."' AND mgd.code='Official_Travel'
		// ), sfao.id_approval_mode, sfao.sequence, sfao.id_detail_chief, sfao.description_chief, sfao.id_employee_approval, he.name
		// FROM sp_funct_approval_organization_hierarchy_view ('".$request->id_employee."','".session('id_company')."',null,Array['gm-level','manager-level']) sfao
		// left JOIN hr_employee he on he.id_employee=sfao.id_employee_approval
		// LIMIT 1
		// ) as hirar
		// join hr_employee he2 
	...
	public static function format_save_to_transaction($request)
	{
		...
		LEFT JOIN master_general_type mgt
		ON mgd.id_general_type = mgt.id_general_type AND mgt.general_type = 'master_approval_doc_type'
		WHERE mgd.id_company = '".session('id_company')."' AND mgd.code='Official_Travel' AND hierarchy_type = 'Organization'
		), sfao.id_approval_mode, sfao.sequence, sfao.id_detail_chief, sfao.description_chief, sfao.id_employee_approval, he.name
		FROM sp_funct_approval_organization_hierarchy_view ('".$request->request_by."','".session('id_company')."',null,Array['gm-level','manager-level']) sfao
		left JOIN hr_employee he on he.id_employee=sfao.id_employee_approval 
		LIMIT 1
		) as han 
	...
	public function send_mail($request)
	{
		...
		LEFT JOIN master_bank ON master_bank.id_bank=hr_bank_employee.id_bank
		WHERE hr_official_travel.id_company = '$id_company' AND hr_approval_transaction.source_transaction_type='Official_Travel' AND hr_approval_transaction.id_company='$id_company' $filter
		";
		$data = DB::select($data);	
		if (!empty($data)) {
	...
	public static function get_uom($idProduct)
	{
		$sql = "SELECT mp.id_uom
				FROM inventory.master_product mp 
				WHERE mp.id_company = ".session('id_company')." AND mp.id_product = ".$idProduct;
