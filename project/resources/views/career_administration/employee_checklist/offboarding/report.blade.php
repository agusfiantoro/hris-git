    ...
    <div class="col-12">
        <div class="card card-danger card-outline">
			...
			<div class="row">
				<div class="col-md-4">
					 <div class="row">
						<label class="col-sm-5 col-form-label">Category Date :</label>
						<div class="col-sm-7">
...
const get_datatable = async () => {
	$(".div_datatable").show();
		...
		var table =	$('#report_table').DataTable({
			...
			buttons: [
				  {
					...
					title: "Report Off Boarding ( <?= session('company_name') ?> )",
					exportOptions: {
						orthogonal: 'export',
						columns: ':visible'
					}
            ...
            ajax: {
				url: "<?= url('career_administration/employee_checklist/offboarding/get_report') . '?id_url=' ?>" + global_url_server,
				"data": myData,
		...
		//rowsGroup: [2],
           columns: [
				...
				{ data: 'compensation_amount', name: 'compensation_amount', render: function (data, type, row) {
                return type === 'export' ? ( data != null ? data.replace( /[$,]/g, '' ) : data ) : (data != null ? parseInt(data).toLocaleString() : data);
					}
				},
                {data: 'status', name: 'status'},
                {data: 'status_emp', name: 'status_emp'},
                {data: 'assigned_hr', name: 'assigned_hr'},
