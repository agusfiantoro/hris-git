  ...
  <div class="col-12">
    <div class="card card-danger card-outline">
      <div class="card-header">
        <h5 class="card-title">P2K Monitoring
        </h5>
      </div>

      <div class="card-body">
			<div class="form-group row" style="border:2px solid #e7e7e7;padding:20px;border-radius:5px;margin-top:8px;">
				<div class="col-md-12">
					<div class="row">
						<div class="col-md-2">
							<label class="col-form-label">Filter Report</label>
						</div>
						<div class="col-md-2">
							<div class="">
						...
						<div class="col-md-2">
							<button type="button" id="download" class="pull-right btn btn-sm btn-success"><i class="fas fa-download" ></i> Report P2K</button>
						</div>
					</div>
				</div>	
			</div>
				...
				<table id="sample_table_review">
					<tr id="" style="font-size:14px;">
						...
						<td>
							...
							<input type="text" name="review[0][period]" id="review_0_period" class="form-control form-control-sm period_input" style="width: 100%;" readonly>
							<span class="invalid-date period_input_error" style="font-size:10px;color:#dc3545;" role="alert" id="review_0_periodError">
								<strong></strong>
							</span>
						</td>
						<td>
							<input type="text" name="review[0][review_date]" id="review_0_review_date" class="form-control form-control-sm review_date_input" style="width: 100%;" disabled>
						...
						<td>
							<input type="text" name="review[0][id_target]" id="review_0_id_target" class="form-control form-control-sm id_target_input" style="font-weight:bold;text-align:center;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" readonly>
							<span class="invalid-feedback id_target_input_error" role="alert" id="review_0_id_targetError">
							  <strong></strong>
							</span>
						</td>
						<td>
							<input type="text" name="review[0][act]" id="review_0_act" class="form-control form-control-sm act_input" style="font-weight:bold;text-align:center;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" readonly>
						...
						<td>
							<select name="review[0][decision]" id="review_0_decision" class="form-control form-control-sm select2 decision_input" style="width: 100%;" readonly>
							</select>
						...
						<td>
							<select name="review[0][treatment]" id="review_0_treatment" class="form-control form-control-sm select2 treatment_input" style="width: 100%;" readonly>
							</select>
...
$(document).on('click', '#new_rec_detail', function () {
		...
		element.find('.review_date_input').parent().children('span').children('button').attr('disabled',true);
	...
	//	element.find('.target_input').html(global_threshold);
		...
		element.find('.pdf_input').attr('id', 'review_' + size + '_pdf');
...
$(document).on('change', '.decision_input', (element) => {
...
function on_close_modal() {
  ...
  $(function () {
    ...
    $('#kpk_table').DataTable({
      ...
      responsive: true,        
      ajax: {
        url: "{{ route('kpk.index_monitoring') }}",
...
//    let status = ['A'];
    ...
    let result;
    try {
        result = await $.getJSON('<?= url('e-letter/performance_plan/performance_monitoring/get_employee_search') ?>'+'?id_url='+global_url_server+'&code_dept='+$('#dept_search').val()+'&status='+$('#status').val(), 
		function (res) {
			$('#loader').addClass('hidden');	
        });
        return result;
...
function getDeptAndEmployee() {
	$.ajax({
		url: "{{route('kpk.get_dept_search')}}",
		success: (res) => {
			$('#dept_search').empty();
			$('#dept_search').prepend('<option selected></option>').select2({
