<script type="text/javascript">
  $(".new").click(function() {
    ...
    $(".select_search").empty();
    $("#modal_form_swp").modal('show');
    $(".invalid-feedback").children("strong").text("");
    $("#swpForm input").removeClass("is-invalid");
    $("#swpForm select").removeClass("custom-select");
    $("#swpForm textarea").removeClass("is-invalid");
    hide_loading();
  });
  $(".select_opsi").select2({
    allowClear: true,
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.status == 'true') {
            ...
            $("#id_position_routing_chief").empty();
            // $(".select_opsi").val(null).trigger('change');
            $('#modal_form_swp').modal('hide');
            swal({
              icon: 'success',
              title: 'Success',
              text: 'Stern Warning Letter Succesfully !!'
            });
