...
Dear, Bpk/Ibu {{$param['content_title']}}
 ...
 Dengan ini kami beritahukan bahwa Anda masuk sebagai PESERTA PROGRAM PERBAIKAN KINERJA (P2K), Sebagaimana ditetapkan dalam BA PENETAPAN PESERTA PROGRAM PERBAIKAN KINERJA (P2K).<br>Berikut BA yang dimaksud:  
 </div>
 ...
 @component('mail::panel')
	...
	<table width="100%" cellpadding="2" cellspacing="2" role="presentation"  style="font-size:14px;border-collapse: collapse;
		...
		<tr style="border: 1px solid #aaa;padding:3px;">
			<td style="border-right: 1px solid #aaa;padding:3px;text-align:center;">{{ $param['obj_kpi'] }}</td>
			<?php if($param['dept_code'] == '170_SAL'){ ?>
					<td style="border-right: 1px solid #aaa;padding:3px;text-align:center;">{{ $param['act_idx'] }}</td>
			<?php } ?>
			<td style="border-right: 1px solid #aaa;padding:3px;text-align:center;">{{ $param['act_kpi'] }}</td>
		</tr>
	</table>
	@endcomponent
