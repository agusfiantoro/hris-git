  ...
  <div class="col-12">
    <div class="card card-danger card-outline">
      <div class="card-header">
        <h5 class="card-title">Bgen Sales Code
        </h5>
      </div>
      <div class="card-body">
			...
			<table id="sample_table_rec">
				<tr id="" style="font-size:14px;">
					...
					<td>
						...
						<select name="bgen[0][id_transition_category]" id="bgen_0_id_transition_category" class="form-control form-control-sm select2 id_transition_category_input" style="width: 100%;"></select>
						<span class="invalid-feedback id_transition_category_input_error" role="alert" id="bgen_0_id_transition_categoryError">
							<strong></strong>
						</span>	
					</td>
					<td>
					...
					<td>
						<!--select name="bgen[0][bgen_position]" id="bgen_0_bgen_position" class="form-control form-control-sm select2 bgen_position_input" style="width: 100%;"></select>
						<span class="invalid-feedback bgen_position_input_error" role="alert" id="bgen_0_bgen_positionError">
							<strong></strong>
						</span -->
						<input name="bgen[0][bgen_position]" id="bgen_0_bgen_position" type="hidden" class="bgen_position_input">
						<div id="bgen_0_bgen_desc_position" class="bgen_desc_position_input"></div>
					</td>
					<td>
						<!-- select name="bgen[0][bgen_location]" id="bgen_0_bgen_location" class="form-control form-control-sm select2 bgen_location_input" style="width: 100%;"></select>
						<span class="invalid-feedback bgen_location_input_error" role="alert" id="bgen_0_bgen_locationError">
							<strong></strong>
						</span -->
						<input name="bgen[0][bgen_location]" id="bgen_0_bgen_location" type="hidden" class="bgen_location_input">
						<div id="bgen_0_bgen_desc_location" class="bgen_desc_location_input"></div>
					</td>
					<td>
					...
					<td align="center">
						<div id="bgen_0_sales_code" class="sales_code_input" style="font-weight:bold;"></div>
					</td>
					<td align="center">
						<div id="bgen_0_direct_spv" class="direct_spv_input"></div>
					</td>
					<td>
					...
					<td>
						<div id="bgen_0_group_id_approval_request" class="group_id_approval_request">
							<select id="bgen_0_id_approval_request" name="bgen[0][id_approval_request]" class="form-control form-control-sm select2 id_approval_request_input" style="width: 100%;"></select>
							<span class="invalid-feedback id_approval_request_error" role="alert" id="bgen_0_id_approval_requestError">
								<strong></strong>
							</span>
						</div>
					</td>
					<td>
						<select id="bgen_0_id_direct_spv" name="bgen[0][id_direct_spv]" class="form-control form-control-sm select2 id_direct_spv_input" style="width: 100%;"></select>
						<span class="invalid-feedback id_direct_spv_error" role="alert" id="bgen_0_id_direct_spvError">
							<strong></strong>
						</span>
					</td>
					<td>
...
function loadedit(id_employee,nik_employee,type,join_date){
	...
	if(join_date > now){
		...
		$("#modal-title").html("<span class='fas fa-edit'></span> Edit Sales Code");
		$.ajax({
				url: "{{ route('bgen.modal_detail') }}",
				data:{
...
$(document).on('click', '#new_rec_detail', function () {
			...
			element.find('.cat_code_input').attr('id', 'bgen_' + size + '_cat_code');
            element.find('.cat_code_input').attr('name', 'bgen[' + size + '][cat_code]'); 
			
			element.find('.id_transition_category_input').attr('id', 'bgen_' + size + '_id_transition_category');
            element.find('.id_transition_category_input').attr('name', 'bgen[' + size + '][id_transition_category]');
            element.find('.id_transition_category_input_error').attr('id', 'bgen_' + size + '_id_transition_categoryError');
            element.find('.id_transition_category_input').prepend('<option selected></option>').select2({
			...
			//	element.find('.group_id_direct_spv').hide();
				...
				if(global_category.length > 0){
					if($(this).select2('data')[0].code == 'Join'){
					...
					//	element.find('.group_id_direct_spv').show();
						...
						element.find('.cat_code_input').val($(this).select2('data')[0].code);
...
function getDeptAndEmployee() {
	$.ajax({
		...
		success: (res) => {
			...
			let found = false;
			res.forEach((dept) => {
				if(!found && dept.department_code == '170_SAL') {
					$('#dept_search').val(dept.id).trigger('change');
					found = true;
				}
			})
	...
	swal({
        ...
        if (value) {	 
			...
			$.ajax({
				...
				url: global_id_integration_sales_code == '' ? "<?= url('integration/bgen/sales_code_bgen/save') . '?counter=' ?>" + counter + "<?= '&id_employee='?>" + global_employee : "<?= url('integration/bgen/sales_code_bgen/update') . '?id_integration_sales_code=' ?>" + global_id_integration_sales_code + "<?= '&counter='?>" + counter,
				data: formData,
				beforeSend: function () {
					$('#loader').removeClass('hidden');
				},
