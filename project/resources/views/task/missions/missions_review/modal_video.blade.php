		...
		<table style="width:100%;font-size:14px;" id="table_photo" class="table table-striped table-bordered table-hover datatable">
			<thead>	
				<tr class="text text-center">
					...
					<th data-priority="2">Video</th>
					<th data-priority="4" style="width:150px;">Note</th>
				</tr>
			</thead>
		</table>
	</div>
		...
		columns: [								
			...
			{data : "attachment", name: 'attachment' , className: 'text-center',
				render: function ( data, type, row ) { 
					let img_path = "<?= url('project/storage/app/public/task/video/') ?>";
					let img_folder  = row.nik_employee;
					let img_file  = data;
					let created_date  = row.creation_date;
					
					show_in = `<video width="320" height="240" controls><source src="${img_path}/${img_folder}/${img_file}" type="video/webm"></video><br>${created_date}`;
					return show_in;
				} 
			},
