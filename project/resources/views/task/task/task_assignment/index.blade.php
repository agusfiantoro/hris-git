		 ...
		 <div style="display:none;">
			<table id="sample_table_rec">
				<tr id="" style="font-size:12px;">
					...
					<td align="center">
						<span id="task_0_generated_date" class="generated_date_input"></span>
					</td>
...
$(document).on('click', '#new_rec_detail', function () {
		...
		*/	
			...
			element.find('.generated_date_input').attr('id', 'task_' + size + '_generated_date');
					...
					get_task_list(arr_task).then(function(res) {
						...
						if($('#table_rec_body tr').length == 0){
							$.each(res, function (i, item) {
								...
								if(item.task_type == 'Mandatory'){
									global_random_type = [{id:'All',text:'Mandatory'}];
								}
								else if(item.task_type == 'Random'){
									global_random_type = [
										{
											id: 'Automatic',
											text: 'Random'
						...
						*/
							$.each(res, function (i, item) {
								...
								if(item.task_type == 'Mandatory'){
									global_random_type = [{id:'All',text:'Mandatory'}];
									siz_task = $('#task_'+item.id_activity+'_id_task').attr('id_task');
									y.push(parseInt(siz_task));
									if ($.inArray(parseInt(item.id_task), y) == -1){
										$('#new_rec_detail').trigger('click');
									}
								}
								else if(item.task_type == 'Random'){
									...
									siz_act = $('#task_'+item.id_activity+'_id_activity').attr('id_activity');
									x.push(parseInt(siz_act));
								...
								$.each(global_z, function (i, item) {
									if(res[i].task_type == 'Mandatory'){
										...
										if ($.inArray(res[i].id_task, y) == -1){
											...
											$('#task_'+item+'_id_task').attr('id_task',res[i].id_task);
											$('#task_'+item+'_random_type').attr("readonly",true);
											$('#task_'+item+'_random_type').val('All').trigger('change');
											$('#task_'+item+'_id_task').attr('task_type','Mandatory');
											if(res[i].activity.length > 1){
												var strAct = [];
												var strEvi = [];
												var strType = [];
												var strCycle = [];
												var strTime = [];
												var strScore = [];
												$.each(res[i].activity, function (i, item) {
													var str_act = "<li>"+item+"</li>";
													strAct.push(str_act);
												});
												$('#task_'+item+'_activity_text').html('<div align="left"><ol style="margin-left:-25px;">'+strAct.join(" ")+'</ol></div>');
												
												$.each(res[i].target_evidence, function (i, item) {
													var str_evi = "<li>"+item+"</li>";
													strEvi.push(str_evi);
												});
												$('#task_'+item+'_evidence').html('<div align="left"><ol style="margin-left:-25px;">'+strEvi.join(" ")+'</ol></div>');
												
												$.each(res[i].evidence_type, function (i, item) {
													var str_type = "<li>"+item+"</li>";
													strType.push(str_type);
												});
												$('#task_'+item+'_type').html('<div align="left"><ol style="margin-left:-25px;">'+strType.join(" ")+'</ol></div>');
												
												$.each(res[i].task_cycle, function (i, item) {
													var str_cycle = "<li>"+item+"</li>";
													strCycle.push(str_cycle);
												});
												$('#task_'+item+'_cycle').html('<div align="left"><ol style="margin-left:-25px;">'+strCycle.join(" ")+'</ol></div>');
												
												$.each(res[i].time, function (i, item) {
													var str_time = "<li>"+item+"</li>";
													strTime.push(str_time);
												});
												$('#task_'+item+'_time').html('<div align="left"><ol style="margin-left:-25px;">'+strTime.join(" ")+'</ol></div>');
												
												$.each(res[i].maximum_score, function (i, item) {
													var str_score = "<li>"+item+"</li>";
													strScore.push(str_score);
												});
												$('#task_'+item+'_score').html('<div align="left"><ol style="margin-left:-25px;">'+strScore.join(" ")+'</ol></div>');
											}
											else{
