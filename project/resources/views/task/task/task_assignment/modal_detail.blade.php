									...
									<table id="table_rec_detail" style="width:100%" class="table table-striped table-bordered table-hover datatable">
										<thead>
											<tr align="center" style="font-size:13px;">
												...
												<th style="width:50px;">Generated Date</th>
			...
			get_dept().then(function(res) {
				...
				setTimeout(function () {		
					$('#table_rec_body tr').each(function (index) {
						...
						$(this).find('.id_task_input').attr('task_type',response.res[index].task_type);
						$(this).find('.random_type_input').attr("readonly",true);
						if(response.res[index].random_type == 'All'){
							$(this).find('.id_task_input').attr('id_task',response.res[index].id_task);						
						}
						else{
							$(this).find('.id_activity_input').attr('id_activity',response.res[index].id_activity);
						}
						...
						if(response.res[index].activity_text.length > 1){
							var strAct = [];
							var strEvi = [];
							var strType = [];
							var strCycle = [];
							...
							$.each(response.res[index].activity_text, function (i, item) {
								var str_act = "<li>"+item+"</li>";
								strAct.push(str_act);
							});
							
							$.each(response.res[index].evidence, function (i, item) {
								var str_evi = "<li>"+item+"</li>";
								strEvi.push(str_evi);
							});
							
							$.each(response.res[index].type, function (i, item) {
								var str_type = "<li>"+item+"</li>";
								strType.push(str_type);
							});						
							
							$.each(response.res[index].cycle, function (i, item) {
								var str_cycle = "<li>"+item+"</li>";
								strCycle.push(str_cycle);
							});
							
							$.each(response.res[index].time, function (i, item) {
								var str_time = "<li>"+item+"</li>";
								strTime.push(str_time);
							});
							
							$.each(response.res[index].score, function (i, item) {
								var str_score = "<li>"+item+"</li>";
								strScore.push(str_score);
							});
							
							$(this).find('.activity_text_input').html('<div align="left"><ol style="margin-left:-25px;">'+strAct.join(" ")+'</ol></div>');
							$(this).find('.evidence_input').html('<div align="left"><ol style="margin-left:-25px;">'+strEvi.join(" ")+'</ol></div>');
							$(this).find('.type_input').html('<div align="left"><ol style="margin-left:-25px;">'+strType.join(" ")+'</ol></div>');
							$(this).find('.cycle_input').html('<div align="left"><ol style="margin-left:-25px;">'+strCycle.join(" ")+'</ol></div>');
							$(this).find('.time_input').html('<div align="left"><ol style="margin-left:-25px;">'+strTime.join(" ")+'</ol></div>');
							$(this).find('.score_input').html('<div align="left"><ol style="margin-left:-25px;">'+strScore.join(" ")+'</ol></div>');
						}
						else{
						...
						$(this).find('.generated_date_input').html(response.res[index].last_date_generated);
					});	
				}, 500);	
