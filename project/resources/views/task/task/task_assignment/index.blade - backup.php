    ...
    <div class="modal-content">
		<form method="POST" id="taskForm">
		 ...
		 <div style="display:none;">
			<table id="sample_table_rec_task">
				<tr id="" style="font-size:12px;">
					<td align="center"><span class="sn" style="vertical-align:middle;text-align:center;"></span></td>
					<td>
						<input name="c_task[0][id_task_activity]" id="c_task_0_id_task_activity" type="hidden" class="form-control form-control-sm id_task_activity_input">
						
						<input name="c_task[0][id_task]" id="c_task_0_id_task" type="hidden" class="form-control form-control-sm id_task_input">
						<span id="c_task_0_task_text" class="task_text_input"></span>
					</td>
					<td>
							<select name="c_task[0][random_type]" id="c_task_0_random_type" class="form-control form-control-sm select2 random_type_input" style="width: 100%;font-size:10px;"></select>
							<span class="invalid-feedback random_type_input_error" role="alert" id="c_task_0_random_typeError">
								<strong></strong>
							</span>	
					</td>
					<td>							
							<input name="c_task[0][id_activity]" id="c_task_0_id_activity" type="hidden" class="form-control form-control-sm id_activity_input">
							<span id="c_task_0_activity_text" class="activity_text_input"></span>
					</td>
					<td align="justify">
						<span id="c_task_0_evidence" class="evidence_input"></span>
					</td>
					<td align="center">
						<span id="c_task_0_type" class="type_input"></span>
					</td>
					<td align="center">
						<span id="c_task_0_cycle" class="cycle_input"></span>
					</td>
					<td align="center">
						<span id="c_task_0_time" class="time_input"></span>
					</td>
					<td align="center">
						<span id="c_task_0_score" class="score_input"></span>
					</td>
...
$(document).on('click', '#new_rec_detail_task', function (event,item) {
            var content = jQuery('#sample_table_rec_task tr'),
               //     size = global_id_rec_detail++,
                    size = item.task,
                    element = null,
                    element = content.clone();
            element.attr('id','rec-'+size);
			element.attr('id_record_task', size);
            element.find('.delete-record').attr('data-id', size);
			
            element.find('.id_task_activity_input').attr('id', 'c_task_' + size + '_id_task_activity');
            element.find('.id_task_activity_input').attr('name', 'c_task[' + size + '][id_task_activity]');
			
			element.find('.id_task_input').attr('id', 'c_task_' + size + '_id_task');
            element.find('.id_task_input').attr('name', 'c_task[' + size + '][id_task]');
		...
		*/	
			...
			element.find('.random_type_input').attr('id', 'c_task_' + size + '_random_type');
            element.find('.random_type_input').attr('name', 'c_task[' + size + '][random_type]');
			element.find('.random_type_input_error').attr('id', 'c_task_' + size + '_random_typeError');
			...
			element.find('.id_activity_input').attr('id', 'c_task_' + size + '_id_activity');
            element.find('.id_activity_input').attr('name', 'c_task[' + size + '][id_activity]');
        //   element.find('.id_activity_input_error').attr('id', 'c_task_' + size + '_id_activityError');
        /*  element.find('.id_activity_input').prepend('<option selected></option>').select2({
                placeholder: "Select Activity ...",
				data: global_id_activity,
            });
		*/	
			element.find('.task_text_input').attr('id', 'c_task_' + size + '_task_text');
			element.find('.activity_text_input').attr('id', 'c_task_' + size + '_activity_text');
			element.find('.evidence_input').attr('id', 'c_task_' + size + '_evidence');
			element.find('.type_input').attr('id', 'c_task_' + size + '_type');
			element.find('.cycle_input').attr('id', 'c_task_' + size + '_cycle');
			element.find('.time_input').attr('id', 'c_task_' + size + '_time');
			element.find('.score_input').attr('id', 'c_task_' + size + '_score');
...
$(document).on('click', '#new_rec_detail', function (event,item) {
            var content = jQuery('#sample_table_rec tr'),
               //     size = global_id_rec_detail++,
                    size = item.id_activity,
                    element = null,
                    element = content.clone();
            element.attr('id','rec-'+size);
			element.attr('id_record_act', size);
            element.find('.delete-record').attr('data-id', size);
...
const get_list_datatable = async () => {
		...
		let btnSubmit = {
			...
			action: function (e, dt, node, config) {
				let arr_task = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
					return {'id_task' : $(entry).attr('id_task'), 'task_type' : $(entry).attr('task_type')};
				});
				if(arr_task.length > 0){
					$('#loader').removeClass('hidden');
					
					get_task_list(arr_task).then(function(res) {
						let x = [];
						let y = [];
						let global_z = [];
						let global_y = [];
						if($('#table_rec_body tr').length == 0){
							$.each(res, function (i, item) {
								if(item.task_type == 'Random'){
									$('#new_rec_detail').trigger('click',[item]);
									global_z.push(item.id_activity);
								}
								if(item.task_type == 'Mandatory'){
									$('#new_rec_detail_task').trigger('click',[item]);
									global_y.push(item.task);
								}
						...
						/*
							$('#table_rec_body tr').each(function (index) {
								siz_type = $(this).find('.id_task_input').attr('task_type');
								if(siz_type == 'Mandatory'){
									siz_task = $(this).find('.id_task_input').attr('id_task');
									y.push(parseInt(siz_task));
								}
								else if(siz_type == 'Random'){
									siz_act = $(this).find('.id_activity_input').attr('id_activity');
									x.push(parseInt(siz_act));
								}
							});
						*/
							$.each(res, function (i, item) {
								if(item.task_type == 'Random'){
								siz_id = $('#task_'+item.id_activity+'_id_activity').attr('id_activity');
								//	if ($.inArray(parseInt(item.id_activity), x) == -1){
									if(siz_id != item.id_activity){
										$('#new_rec_detail').trigger('click',[item]);
										global_z.push(item.id_activity);
										console.log('Tes');
									}
								...
								if(item.task_type == 'Mandatory'){
								siz_id = $('#task_'+item.id_task+'_id_task').attr('id_task');
								//	if ($.inArray(parseInt(item.id_activity), x) == -1){
									if(siz_id != item.id_task){
										$('#new_rec_detail_task').trigger('click',[item]);
										global_y.push(item.id_task);
									}
								//	}
								}
							});		
					...
					//	get_task_list(id_task).then(function(res) {
						if(global_z.length > 0){
							setTimeout(function () {
								$.each(global_z, function (i, item) {
									if(res[i].task_type == 'Random' ){
										act_id = $('#task_'+item+'_id_activity').attr('id_activity');
										if (act_id != res[i].id_activity){	
											...
											$('#task_'+item+'_id_activity').attr('id_activity',res[i].id_activity);
											$('#task_'+item+'_random_type').val('Automatic').trigger('change');
						...
						if(global_y.length > 0){
							setTimeout(function () {
								$.each(global_y, function (i, item) {
									if(res[i].task_type == 'Mandatory' ){
										act_id = $('#task_'+item+'_id_task').attr('id_task');
										if (act_id != res[i].id_task){	
											$('#c_task_'+item+'_id_task').val(res[i].id_task).trigger('change',[true]);
											$('#c_task_'+item+'_id_task').attr('id_task',res[i].id_task);
											$('#c_task_'+item+'_id_activity').val(res[i].id_activity).trigger('change',[true]);										
											$('#c_task_'+item+'_task_text').html(res[i].name_task);
										//	$('#c_task_'+item+'_id_activity').attr('id_activity',res[i].id_activity);
											$('#c_task_'+item+'_random_type').val('All').trigger('change');
											$('#c_task_'+item+'_activity_text').html(res[i].activity);
											$('#c_task_'+item+'_evidence').html(res[i].target_evidence);
											$('#c_task_'+item+'_type').html(res[i].evidence_type);
											$('#c_task_'+item+'_cycle').html(res[i].task_cycle);
											$('#c_task_'+item+'_time').html(res[i].time);
											$('#c_task_'+item+'_score').html(res[i].maximum_score);
										}
									}
									$('#loader').addClass('hidden');
								});
		...
		var t = $('#list_table').DataTable({
			...
			createdRow: function( row, data, dataIndex ) {
			  ...
			  $(row).attr('task_type', data['task_type']);
			},
...
const get_task_list = async (arr_task) => {
	let result;
	let myData = {
		//	id_pos: id_position,
			arr_task: arr_task,
