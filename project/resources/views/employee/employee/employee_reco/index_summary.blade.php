			...
			<table id="reco_table" style="width: 100%;" class="table table-striped table-bordered table-hover datatable">
			  <thead>
			   <tr>
				...
				<th data-priority="11">Current Position</th>
				<th data-priority="12">Branch</th>
		...
		<form method="POST" id="recoForm">
		  ...
		  <div class="modal-body" style="height: 500px;overflow-y: auto;">
				...
				<div class="row">                                      
					<div class="col-12">
						<div class="card-body">
							<h5 style="font-weight:bold;margin-bottom:10px;">List Approval</h5>
...
function loadedit(id_recommendation_header,type,sum){
	...
	if(type == 'view'){
		...
		$("#save_and_draft").show().html('<i class="fas fa-save"></i> Save & Draft').removeClass('addForm').addClass('editForm');
...
const get_datatable_employee = async () => {	
	...
	$('#reco_table').DataTable({
		...
		responsive: true,     
	ajax: {
        url: "{{ route('reco_form.index_summary') }}",
...
function get_reco_check(type,datenow){
	...
	//	$('#loader').removeClass('hidden');		
			...
			if(this.checked){									
				...
				if(type == 'new'){
					$("#save_and_submit").hide();
					$("#save_and_draft").show().html('<i class="fas fa-save"></i> Save & Draft').removeClass('editForm').addClass('addForm');
				}			
			}
			else {
				$('#id_reco_check').hide();
...
function list_approval(id_source_transaction,source_transaction_type){
	$.extend( true, $.fn.dataTable.defaults, {
		 columnDefs:false,
		 paging:false,
		 searching:false,
	...
	//	processing: true,
		ajax: {
			url: "<?= url('employee/employee/employee_approval/index_status').'?id_source_transaction='?>"+id_source_transaction+"<?= '&source_transaction_type='?>"+source_transaction_type,							
		},
		columns: [
			...
			{data: 'execute', name: 'execute', className: 'text-center'},
		]
	});
}
