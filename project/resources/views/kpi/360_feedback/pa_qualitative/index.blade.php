			...
			<div class="card-body">
				<div class="form-group row">              
					...
					<div class="col-sm-5">
						...
						<button type="button" id="download_detail" class="btn btn-sm btn-success pull-right"><i class="fas fa-download" ></i> Export 360 Subordinat</button>
                    </div>
                </div>
...
let global_emp_name = "";
 $(document).on('click', '.edit', function () {
        let id_qualitative_appraisers = $(this).attr('id');
        ...
        $.ajax({
            url: "<?= url('kpi/360_feedback/pa_qualitative/get_appraiser_edit') ?>",
            method: "GET",
...
//	localStorage.setItem("id_period_qualitative", $("#period").val());	
    get_datatable();
});
...
function load_detail(){
	 $("#contentBody").html('');
	let global_period = $("#period").val();
    $.ajax({
		url: "{{ route('appraisal.review') }}",
		method: "GET",
        data: {period_code: global_period},
...
function get_period() {
	$.getJSON('<?= url('kpi/360_feedback/pa_qualitative/get_period') ?>', function (data) {
			$('#period').select2({
                data: data
            });
		}).then(function (data){
...
$(document).on('click', '#download_detail', function () {
	...
	$.ajax({
		...
		url:"{{ route('appraisal.export_validate') }}",
		data:myData,
		dataType:'json',
		beforeSend: function () {
			$('#loader').removeClass('hidden');		
		...
		success:function(msg){ 
			...
			let url = "{{ url('kpi/360_feedback/export_detail') }}";	
