...
@section('title', 'Organization Unit')
        ...
        <div class="card card-danger card-outline">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <button type="button" class="new btn btn-sm btn-success"><i class="fas fa-plus"></i> Add Department</button>
                </div>
            </div>
			 ...
			 <button id="advanced" type="button" class="btn btn-default">Advanced Search</button>
					<br>
					<br>
					<table id="ou_table" style="width:100%;" class="table table-striped table-bordered table-hover datatable">
					 <thead>
					  <tr>				   
						<th></th>
						<th>No</th>
...
<div class="modal fade" id="modal_form_ou"  data-backdrop="static" data-keyboard="false" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" id="ouForm">
                {{ csrf_field() }}
                <div class="modal-header">
                    <h5 class="modal-title">Form Department</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                <label class="col-sm-4 col-form-label">Department Code</label>
                                <div class="col-sm-8">
                                    <input type="text" name="department_code" id="department_code" class="form-control form-control-sm">
                                    <span class="invalid-feedback" role="alert" id="department_codeError">
                                        <strong></strong>
                                    </span>                                    
                                </div>
                            </div>
					...
					<div class="row">
                         <div class="col-md-6">
                            <div class="row">
                                <label class="col-sm-4 col-form-label">Department Code</label>
                                <div class="col-sm-8">
                                    <input type="text" name="department_code" id="department_code_edit" class="form-control form-control-sm">
                                    <span class="invalid-feedback" role="alert" id="department_code_editError">
                                        <strong></strong>
                                    </span>                                    
                                </div>
                            </div>
...
$(document).on('click', '.new', function () {
            $("#ouForm")[0].reset();
            $("#ouForm .modal-title").html("<span class='fas fa-plus'></span> Form Organization Unit");
            $(".invalid-feedback").children("strong").text("");
            $("#ouForm input").removeClass("is-invalid");
            $("#ouForm textarea").removeClass("is-invalid");
            $('#modal_form_ou').modal('show');
        });

    $(function () {
        $('#ouForm').submit(function (e) {
            e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status == 'true') {
							 $('#modal_form_ou').modal('hide');
                            swal({
                                icon: 'success',
                                title: 'Success',
                                text: response.message
                            });
	                        $('#ou_table').DataTable().ajax.reload();
                        } else {
                            swal({
                                icon: 'error',
                                title: 'Oops...',
                        ...
                        if (response.status === 422) {
                            ...
                            Object.keys(errors).forEach(function (key) {
							 ...
							 $("#department_code").addClass("is-invalid");
                              $("#department_codeError").children("strong").text('The Department Code has already been taken.');
                        }
						else {
                            swal({
                                icon: 'error',
...
$(document).ready(function(){
  ...
  $("#update_form input").removeClass("is-invalid");
   ...
   error: function (response) {
		if (response.status === 422) {
			...
			$("#department_code_edit").addClass("is-invalid");
            $("#department_code_editError").children("strong").text('The Department Code Code has already been taken.');
		}
	}
  });
 });
 ...
 $(document).on('click', '.edit', function(){
  ...
  $('#form_result').html('');
  $.ajax({
   url :"organization_unit/edit/"+id_dept,
   dataType:"json",
   success:function(data)
   {
    $('#department_code_edit').val(data.result.department_code);
...
$(document).on('click', '.delete', function (event) {
    ...
    swal({
       ...
       buttons: true,
        ...
        if (value) {
            $.ajax({
			   url:"organization_unit/destroy/"+id_dept,
			   success:function(data)
			   {
				setTimeout(function(){
