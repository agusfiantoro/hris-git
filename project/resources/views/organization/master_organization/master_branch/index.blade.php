    ...
    <div class="col-12">
        ...
        <div class="card card-danger card-outline">
            <div class="card-header">
                <h5 class="card-title">Master Branch</h5>
                <div class="card-tools">
                    <button type="button" class="new btn btn-sm btn-success"><i class="fas fa-plus"></i> Add Branch</button>
                </div>
            </div>
			 ...
			 <button id="advanced" type="button" class="btn btn-default">Advanced Search</button>
					<br>
					<br>
					<table id="branch_table" style="width:100%;" class="table table-striped table-bordered table-hover datatable">
					 <thead>
					  <tr>					   
						<th></th>
						<th></th>
...
<div class="modal fade" id="modal_form_branch"  data-backdrop="static" data-keyboard="false" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" id="branchForm">
                {{ csrf_field() }}
                <div class="modal-header">
                    <h5 class="modal-title">Form Branch</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    ...
                                    <span class="invalid-feedback" role="alert" id="branch_codeError">
                                        <strong></strong>
                                    </span>                                    
                                </div>
                            </div>
                            <div class="row">
                               ...
                               <div class="col-sm-8">
                                    <select name="id_region" id="region" class="form-control form-control-sm select2" style="width: 100%;">
                                    </select>
                        ...
                        <div class="col-md-6" style="margin-top: 10px">
                            <div class="row">
                                <label class="col-sm-4 col-form-label">ERP Branch Code</label>
                                <div class="col-sm-8">
                                    <input name="id_branch_erp" id="id_branch_erp" class="form-control form-control-sm">
                                    <span class="invalid-feedback" role="alert" id="id_branch_erpError">
                                        <strong></strong>
                                    </span>
                                </div>
                            </div>
					...
					<div class="row">
                         <div class="col-md-6">
                            <div class="row">
                                <label class="col-sm-4 col-form-label">Branch Code</label>
                                <div class="col-sm-8">
                                    <input type="text" name="branch_code" id="branch_code_edit" class="form-control form-control-sm">
                                    <span class="invalid-feedback" role="alert" id="branch_code_editError">
                                        <strong></strong>
                                    </span>                                    
                                </div>
                            </div>
							 <div class="row">
                                <label class="col-sm-4 col-form-label">Region</label>
                               <div class="col-sm-8">
                                    <select name="id_region" id="region_edit" class="form-control form-control-sm select2" style="width: 100%;">
                                    </select>
                                    <span class="invalid-feedback" role="alert" id="region_editError">
                                        <strong></strong>
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="col-md-6" style="margin-top: 10px">
                            <div class="row">
                                <label class="col-sm-4 col-form-label">ERP Branch Code</label>
                                <div class="col-sm-8">
                                    <input name="id_branch_erp" id="id_branch_erp_edit" class="form-control form-control-sm">
                                    <span class="invalid-feedback" role="alert" id="id_branch_erp_editError">
                                        <strong></strong>
                                    </span>
                                </div>
                            </div>
...
function get_company() {
          $.getJSON('<?= url('organization/master_organization/master_branch/get_company') ?>', function (data) {
            $('#company').select2({
                data: data,
				disabled: true
            });
...
function get_regional() {	
		$.getJSON('<?= url('organization/master_organization/master_branch/get_regional') ?>', function (data) {
            $('#region').select2({
                data: data,
            });
			$('#region_edit').select2({
...
$(document).on('click', '.new', function () {
            $("#branchForm")[0].reset();
            $("#branchForm .modal-title").html("<span class='fas fa-plus'></span> Form Branch");
            $(".invalid-feedback").children("strong").text("");
            $("#branchForm input").removeClass("is-invalid");
            $("#branchForm textarea").removeClass("is-invalid");
            $('#modal_form_branch').modal('show');
        });
                        ...
                        if (response.status === 422) {
                            ...
                            Object.keys(errors).forEach(function (key) {
							 ...
							 $("#branch_code").addClass("is-invalid");
                              $("#branch_codeError").children("strong").text('The Branch Code has already been taken.');
                        }
						else {
                            swal({
                                icon: 'error',
...
$(document).ready(function(){
    ...
    $('#branch_table').DataTable({
		...
		{ data: 'DT_RowIndex', name: 'DT_RowIndex'},
            ...
            { data: 'desc_region', name: 'desc_region' },
            { data: 'company_name', name: 'company_name' },
            { data: 'id_branch_erp', name: 'id_branch_erp' },
   ...
   success:function(data)
   {
    ...
    error: function (response) {
		if (response.status === 422) {
			...
			$("#branch_code_edit").addClass("is-invalid");
            $("#branch_code_editError").children("strong").text('The Branch Code has already been taken.');
		}
	}
  });
 });
 ...
 $(document).on('click', '.edit', function(){
  ...
  $('#form_result').html('');
  $.ajax({
   url :"master_branch/edit/"+id_branch,
   dataType:"json",
   success:function(data)
   {
    $('#branch_code_edit').val(data.result.branch_code);
    $('#description_edit').val(data.result.description);
    $('#region_edit').val(data.result.id_region).trigger('change');
    $('#id_company_edit').val(data.result.id_company).trigger('change');
    $('#datepicker_edit').val(data.result.inactive_date);
    $('#hidden_id').val(id_branch);
    $('#id_branch_erp_edit').val(data.result.id_branch_erp).trigger('change');
...
$(document).on('click', '.delete', function (event) {
    ...
    swal({
       ...
       buttons: true,
        ...
        if (value) {
            $.ajax({
			   url:"master_branch/destroy/"+id_branch,
			   success:function(data)
			   {
				setTimeout(function(){
