    ...
    <div class="col-12">
        <div class="card card-danger card-outline">
            <div class="card-header">
                <h5 class="card-title">Asset Revaluation
                </h5>
                ...
                <div class="card-tools">
                    ...
                    <button type="button" more_type="New" class="new btn btn-sm btn-success"><i class="fas fa-plus"></i> Add Revaluation Request</button>
                </div>
            </div>
            <div class="card-body">
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalEditTitle">Add Asset Revaluation Request</h5>
            </div>
            <div class="modal-body">
                <form id="assetGroupForm" method="post">
                    <div class="row">
                        {{ csrf_field() }}
                        <input type="hidden" name="id_revaluation_header" id="id_revaluation_header" class="pk">
                        ...
                        <div class="col cont">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col cont">
                                    <input type="text" name="revaluation_date" id="revaluation_date" class="form-control form-control-sm store-change date" style="width:100%" disabled>
                                    <span class="error"></span>
                                </div>
                            </div>
                            <div class="row mb-2">
                                    ...
                                    <table id="assignedEmployeeTable" class="table table-hover table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width:150px;" class="asset-group">Revaluation Amount</th>
                                                <th style="width:150px;" class="category-group">Revaluation Percentage</th>
...
<table id="sampleDetailTable" style="display:none;">
    <tbody>
        <tr>
            ...
            <td class="detail_id_revaluation_detail_source cont category-group">
                <input type="hidden" name="detail[][id_revaluation_detail]" class="detail-id_revaluation_detail-input">
                <select name="detail[][id_asset_category]" class="form-control form-control-sm store-value detail-id_asset_category-input category-group" style="width:100%"></select>
                <span class="error"></span>
            </td>
            <td class="detail_id_asset cont asset-group">
                <select name="detail[][id_asset]" class="form-control form-control-sm store-value detail-id_asset-input asset-group" style="width:100%"></select>
                <span class="error"></span>
            </td>
            <td class="detail_revaluation_amount cont asset-group">
                <input name="detail[][revaluation_amount]" class="form-control form-control-sm store-value detail-revaluation_amount-input money asset-group">
                <span class="error"></span>
            </td>
            <td class="detail_revaluation_percentage cont category-group">
                <input name="detail[][revaluation_percentage]" class="form-control form-control-sm store-value detail-revaluation_percentage-input category-group">
                <span class="error"></span>
            </td>
            ...
            <td class="detail_id_counterpart_account cont">
                <select name="detail[][id_counterpart_account]" class="form-control form-control-sm store-value detail-id_counterpart_account-input" style="width:100%" readonly="true"></select>
                <span class="error"></span>
            </td>
...
<script type="text/javascript">
    ...
    let toModify = ['id_revaluation_detail', 'id_asset_category', 'id_asset', 'revaluation_amount', 'revaluation_percentage', 'id_account', 'id_counterpart_account', 'status'];
    let status = [
        { id: 'A', text: 'Active' },
        { id: 'I', text: 'Inactive' },
    ];
    ...
    function getInitData(modifyDependentElements = true) {
        $.ajax({
            url: "{{ route('assets.revaluations.get_data') }}",
            beforeSend: () => {
    ...
    $(document).on('click', '.new', function() {
        ...
        $('#modalEditTitle').text('Add Asset Revaluation Request');
    ...
    $(document).on('click', '.edit', function() {
        ...
        $.ajax({
            ...
            data: {
                id_revaluation_header: $(this).attr('id-revaluation-header'),
            },
            beforeSend: () => {
                $('.error').text('');
                $('#assignedEmployeeTableBody').empty();
    ...
    $(document).on('change', '.detail-id_asset_category-input', function () {
        if($(this).val()) {
            $(this).closest('tr').find('.detail-id_asset-input, .detail-revaluation_amount-input').val('').trigger('change').attr('disabled', true);
            if(!$('#id_revaluation_header').val()) {
            ...
            $(this).closest('tr').find('.detail-id_asset-input, .detail-revaluation_amount-input').attr('disabled', false);
        }
    ...
    $(document).on('change', '.detail-id_asset-input', function () {
        if($(this).val()) {
            ...
            $(this).closest('tr').find('.detail-id_asset_category-input, .detail-revaluation_percentage-input').val('').trigger('change').attr('disabled', true);
        } else {
            if(!$('#id_revaluation_header').val()) {
                $(this).closest('tr').find('.detail-id_account-input').val(null).trigger('change');
                $(this).closest('tr').find('.detail-id_counterpart_account-input').val(null).trigger('change');
            }
            $(this).closest('tr').find('.detail-id_asset_category-input, .detail-revaluation_percentage-input').attr('disabled', false);
        }
    });
    ...
    $('#assetGroupForm').submit(function(e) {
        e.preventDefault();
        $.ajax({
            url: "{{ route('assets.revaluations.save') }}",
            type: 'POST',
            data: $(this).serialize(),
            beforeSend: () => {
                $('.error').empty();
    ...
    $('#assetGroupTable').DataTable({
        ...
        columns: [
            ...
            { data: 'revaluation_date', name: 'revaluation_date', title: 'Revaluation Date'},
            ...
            { 
                ...
                title: 'Action',
                render: (data, type, row) => {
                    return `<span class="btn btn-primary edit" id-revaluation-header="${data}"><i class="fas fa-edit"></i></span>`;
                }
            }
        ]
    });
