...
@section('title', 'Asset Impairment Approval')
    ...
    <div class="col-12">
        <div class="card card-danger card-outline">
            <div class="card-header">
                <h5 class="card-title">Asset Impairment Approval
                </h5>
...
<script type="text/javascript">
    ...
    function approveOrReject(action, id) { 
        let url = action == 'approve' ? "{{ route('assets.impairment_approval.approve') }}" : "{{ route('assets.impairment_approval.reject') }}";
        $.ajax({
            url: url,
            type: 'POST',
            data: {
                _token: "{{ csrf_token() }}",
                id_impairment_header: id,
            },
            success: (res) => {
                $('#modalAdd').modal('hide');
                $('#assetGroupTable').DataTable().ajax.reload();
    ...
    $(document).on('click', '.approve', function() {
        swal({
            ...
            if(confirm) {
                approveOrReject('approve', $(this).attr('id-impairment-header'));
            }
        })
    });
    ...
    $(document).on('click', '.reject', function() {
        swal({
            ...
            if(confirm) {
                approveOrReject('reject', $(this).attr('id-impairment-header'));
            }
        })
    });
    ...
    $('#assetGroupTable').DataTable({
        responsive: true,
        ajax: {
            url: "{{route('assets.impairment_approval')}}"
        },
        columnDefs: [
        ...
        columns: [
            ...
            { data: 'request_start_date', name: 'impairment_date', title: 'Impairment Date'},
            ...
            { 
                ...
                render: (data, type, row) => {
                    return `
                        <span class="btn btn-success approve" id-impairment-header="${data}"><i class="fas fa-check"></i></span>
                        <span class="btn btn-danger reject" id-impairment-header="${data}"><i class="fas fa-times"></i></span>
                        <span class="btn btn-warning edit" id-impairment-header="${data}"><i class="fas fa-eye" style="color:#fff;"></i></span>
                        `;
                }
            }
        ]
