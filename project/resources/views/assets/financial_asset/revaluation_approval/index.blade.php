...
@section('title', 'Asset Revaluation Approval')
    ...
    <div class="col-12">
        <div class="card card-danger card-outline">
            <div class="card-header">
                <h5 class="card-title">Asset Revaluation Approval
                </h5>
                        ...
                        <table id="assignedEmployeeTable" class="table table-hover table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:150px;">Revaluation Amount</th>
                                    <th style="width:150px;">Revaluation Percentage</th>
...
<table id="sampleDetailTable" style="display:none;">
    <tbody>
        <tr>
            ...
            <td class="detail_id_revaluation_detail_source cont">
            ...
            <td class="detail_revaluation_amount cont">
                <input name="detail[][revaluation_amount]" class="form-control form-control-sm store-value detail-revaluation_amount-input money">
                <span class="error"></span>
            </td>
            <td class="detail_revaluation_percentage cont">
                <input name="detail[][revaluation_percentage]" class="form-control form-control-sm store-value detail-revaluation_percentage-input">
                <span class="error"></span>
            </td>
...
<script type="text/javascript">
    ...
    let periods, assets, assetCategories, employees, idEmployee, branches, retirementHeaders, accounts = [];
    ...
    $(document).on('change', '.detail-id_asset_category-input', function () {
        $.ajax({
            url: "{{ route('assets.revaluations.get_data') }}",
            data: {
                id_asset_category: $(this).val(),
            },
            beforeSend: () => {
                $(this).closest('tr').find('.error').addClass('loading-readonly-element lds-dual-ring overlay');
            },
            success: (res) => {
                $(this).closest('tr').find('.detail-id_asset-input').empty().prepend('<option></option>').select2({ 
                    data: res.data.assets,
                    allowClear: true,
                    placeholder: 'Select Asset',
                });
                if($(this).closest('tr').find('.detail-id_asset-input').attr('default-value')) {
                    $(this).closest('tr').find('.detail-id_asset-input').val(
                        $(this).closest('tr').find('.detail-id_asset-input').attr('default-value')
                    ).trigger('change');
                }
            },
            complete: () => {
                $(this).closest('tr').find('.error').removeClass('loading-readonly-element lds-dual-ring overlay');
            }
        })
    });
    ...
    function approveOrReject(action, id) { 
        let url = action == 'approve' ? "{{ route('assets.revaluation_approval.approve') }}" : "{{ route('assets.revaluation_approval.reject') }}";
        $.ajax({
            url: url,
            type: 'POST',
            data: {
                _token: "{{ csrf_token() }}",
                id_revaluation_header: id,
            },
            success: (res) => {
                $('#modalAdd').modal('hide');
                $('#assetGroupTable').DataTable().ajax.reload();
    ...
    $(document).on('click', '.approve', function() {
        swal({
            ...
            if(confirm) {
                approveOrReject('approve', $(this).attr('id-revaluation-header'));
            }
        })
    });
    ...
    $(document).on('click', '.reject', function() {
        swal({
            ...
            if(confirm) {
                approveOrReject('reject', $(this).attr('id-revaluation-header'));
            }
        })
    });
    ...
    $('#assetGroupTable').DataTable({
        responsive: true,
        ajax: {
            url: "{{route('assets.revaluation_approval')}}"
        },
        columnDefs: [
        ...
        columns: [
            ...
            { data: 'request_start_date', name: 'revaluation_date', title: 'Revaluation Date'},
            ...
            { 
                ...
                render: (data, type, row) => {
                    return `
                        <span class="btn btn-success approve" id-revaluation-header="${data}"><i class="fas fa-check"></i></span>
                        <span class="btn btn-danger reject" id-revaluation-header="${data}"><i class="fas fa-times"></i></span>
                        <span class="btn btn-warning edit" id-revaluation-header="${data}"><i class="fas fa-eye" style="color:#fff;"></i></span>
                        `;
                }
            }
        ]
