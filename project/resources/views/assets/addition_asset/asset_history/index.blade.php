    ...
    <div class="col-12">
        <div class="card card-danger card-outline">
            <div class="card-header">
                <h5 class="card-title">Asset History
                </h5>
                        ...
                        <div class="row mt-5" id="rows_table_cash_advance">
                            <div class="col-xl-12" id="tab_detail">
                              <div class="nav nav-tabs justify-content-left mb-4">
                                ...
                                <a class="nav-item nav-link" id="tab-cost" data-toggle="tab" href="#tab-pane-3">Cost History
                                    <span class="error-tab text-red hidden">Error</span>
                                </a>
                                <a class="nav-item nav-link" id="tab-depreciation" data-toggle="tab" href="#tab-pane-4">Depreciation
                                    <span class="error-tab text-red hidden">Error</span>
                                </a>
                                <a class="nav-item nav-link" id="tab-transferhistory" data-toggle="tab" href="#tab-pane-5">Transfer History
                                    <span class="error-tab text-red hidden">Error</span>
                                </a>
                              </div>
                            </div>
                            <div class="col-xl-12">
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane" id="tab-pane-5">
                                        <div class="table-responsive col-md-12"  style="overflow:auto;">
                                            <table id="transferHistoryTable" class="table table-hover table-bordered table-striped">
                                            </table>
                                        </div>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    function getEdit(idAsset) {
        $.ajax({
            ...
            success: (res) => {
                ...
                $('#transferHistoryTable').DataTable().clear();
                $('#transferHistoryTable').DataTable().rows.add(res.data.assigned_employee_history).draw();
    ...
    function printLabel(id_assets = []) {
        window.open(`{{ route('assets.asset_history.print_label') }}?id_assets=${id_assets}`, "_blank");
        // $.ajax({
        //     url: "{{ route('assets.asset_history.print_label') }}",
        //     type: 'POST',
        //     headers: {
        ...
        //     },
        //     beforeSend: () => {
        //         $('.print-label').attr('disabled', true);
        //         $('.print-label').html('Print Label <div class="spinner-border spinner-border-sm"></div>');
        //     },
        //     complete: () => {
        //         setTimeout(() => {
        //             $('.print-label').attr('disabled', false);
        //             $('.print-label').text('Print Label');
        //         }, 1000);
        //     },
        //     success: (res) => {
        //         $('.print-label').html('Print Label <i class="fas fa-check"></i>');
        //     },
        //     error: (err) => {
        //         $('.print-label').html('Print Label <i class="fas fa-times"></i>');
        //         swal({
        //             icon: 'error',
        //             title: 'Error',
	...
	dtButtons.push({
        text: 'Print Label',
        className: 'btn btn-success print-label',
        action: function (e, dt, node, config) {
            let assets = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
                return $(entry).find('.edit').attr('id-asset')
            });
            if(assets.length < 1){
            	swal({
                    icon: 'warning',
                    title: 'Warning',
                    text: 'Please select Asset first'
                });
                return;
            } 
            printLabel(assets);
    ...
    $('#costHistoryTable').DataTable({
        columns: [
            ...
            { data: 'je_header', title: 'Journal', render: (data, _, row) => data?.reference_number? `<a onclick="loadEditJournal(${row.id_je_header}, '#journalModal')" href="#">${data.reference_number}</a>` : '-' }
        ]
    });
            ...
            columns: [
                ...
                { 
                    ...
                    render: (data, type, row) => {
                        return `<span class="btn btn-primary btn-sm edit" id-asset="${row.id_asset}"><i class="fas fa-edit"></i></span>`;
                    }
                }
            ],
            buttons: dtButtons,
